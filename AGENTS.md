...
- The app supports light, dark, and follow-system themes. All stylesheet colors use `light-dark()`, switched by `color-scheme` via the `html[data-theme]` attribute; never add a raw color literal (see docs/architecture.md "Theming"). The preference lives in localStorage under `ppp-panel-theme` (device-local by design, no backend/device sync) and is changed from the Settings screen's Appearance panel.
